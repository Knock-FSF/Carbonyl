...
public static class MacroParser
{
    ...
    {
        List<(string, string, string, int, int)> macros = new List<(string, string, string, int, int)>();
        // 获取代码的所有行
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            if (lines[i].TrimStart().StartsWith("#"))
            {
                ...
                // 提取宏标识符
                string identifier = content.Substring(0, spaceIndex);
                // 提取宏定义
                string definition = content.Substring(spaceIndex).Trim();
                // 宏指令的位置（字符偏移量）和长度
                int lineNum = i; // 行号
                int length = line.Length; // 行长度
